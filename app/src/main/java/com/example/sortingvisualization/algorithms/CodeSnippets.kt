...
object CodeSnippets {
    private val algorithmCodeSnippets = mapOf(
        SortingAlgorithm.BUBBLE_SORT to """
            fun bubbleSort(arr: MutableList<Double>) {
                val n = arr.size
        ...
        SortingAlgorithm.INSERTION_SORT to """
            fun insertionSort(arr: MutableList<Double>) {
        ...
        SortingAlgorithm.MERGE_SORT to """
            fun mergeSort(arr: MutableList<Double>, left: Int, right: Int) {
                if (left < right) {
            ...
            fun merge(arr: MutableList<Double>, left: Int, mid: Int, right: Int) {
        ...
        SortingAlgorithm.QUICK_SORT to """
            fun quickSort(arr: MutableList<Double>, low: Int, high: Int) {
                if (low < high) {
            ...
            fun partition(arr: MutableList<Double>, low: Int, high: Int): Int {
                val pivot = arr[high]
                var i = low - 1
        ...
        SortingAlgorithm.HEAP_SORT to """
            fun heapSort(arr: MutableList<Double>) {
                val n = arr.size
            ...
            fun heapify(arr: MutableList<Double>, n: Int, i: Int) {
                var largest = i
                val left = 2 * i + 1
                val right = 2 * i + 2
        ...
        SortingAlgorithm.SHELL_SORT to """
            fun shellSort(arr: MutableList<Double>) {
                val n = arr.size
                var gap = n / 2
        ...
        SortingAlgorithm.COCKTAIL_SHAKER_SORT to """
            fun cocktailShakerSort(arr: MutableList<Double>) {
                var swapped: Boolean
                var start = 0
                var end = arr.size
        ...
        SortingAlgorithm.COMB_SORT to """
            fun combSort(arr: MutableList<Double>) {
        ...
        SortingAlgorithm.COUNTING_SORT to """
            fun countingSort(arr: MutableList<Double>) {
        ...
        SortingAlgorithm.RADIX_SORT to """
            fun radixSort(arr: MutableList<Double>) {
                if (arr.isEmpty()) return
            ...
            private fun countingSortByDigit(arr: MutableList<Double>, exp: Int) {
                val n = arr.size
        ...
        SortingAlgorithm.BUCKET_SORT to """
            fun bucketSort(arr: MutableList<Double>) {
                if (arr.isEmpty()) return
    ...
    private val languageCodeSnippets = mapOf(
        ...
        SortingAlgorithm.COCKTAIL_SHAKER_SORT to mapOf(
            ...
            """.trimIndent(),
            ProgrammingLanguage.JAVA to """
                public void cocktailShakerSort(double[] arr) {
                    boolean swapped;
                    int start = 0;
                    int end = arr.length;
